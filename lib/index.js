...
class ChromeUdpTransport {
  ...
  _udpConnect(port) {
    ...
    return new Promise((resolve, reject) => {
      try {
        chrome.sockets.udp.create({}, _ref => {
          ...
          if (chrome.runtime.lastError) {
            ...
            try {
              chrome.sockets.udp.bind(socketId, `127.0.0.1`, port + PORT_DIFF, result => {
                if (chrome.runtime.lastError) {
                  reject(chrome.runtime.lastError);
                } else {
                  if (result >= 0) {
  ...
  _udpSend(socketId, data) {
    ...
    return new Promise((resolve, reject) => {
      try {
        chrome.sockets.udp.send(socketId, sendData, info.address, info.port, _ref2 => {
          let resultCode = _ref2.resultCode;
  ...
  constructor() {
    ...
    chrome.sockets.udp.onReceive.addListener(_ref3 => {
      let socketId = _ref3.socketId;
      let data = _ref3.data;
...
exports.default = chromeTransport;
